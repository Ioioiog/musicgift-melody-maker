    ...
    return <div className="py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map(i => <div key={i} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
  ...
  const VideoWithOverlay = ({
    ...
    if (type === 'youtube') {
      if (!showYouTubeVideo) {
        ...
        return <div className="relative group overflow-hidden rounded-xl bg-gray-50">
            ...
            <button onClick={() => setShowYouTubeVideo(true)} className="absolute inset-0 w-full h-full bg-black/0 hover:bg-black/5 transition-all duration-200" aria-label="Play YouTube video" />
      ...
      return <div className="relative group overflow-hidden rounded-xl bg-gray-50">
          <iframe className="w-full h-full" src={src} allowFullScreen loading="lazy" title={title} />
          <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-lg p-2 shadow-sm">
            <ExternalLink className="w-4 h-4 text-red-600" />
          </div>
        </div>;
    ...
    return <div className="relative group overflow-hidden rounded-xl bg-gray-50">
        <video className="w-full h-full object-cover" autoPlay muted={isMuted} loop playsInline preload="metadata">
        ...
        <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-lg p-2 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <button onClick={() => setIsMuted(!isMuted)} className="text-gray-600 hover:text-gray-800 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          return <motion.div key={testimonial.id} initial={{
            ...
            delay: index * 0.05
                ...
                {type === 'text-only' && <div className="bg-white rounded-2xl border border-gray-100 hover:border-gray-200 transition-all duration-300 p-8 h-full flex flex-col group hover:shadow-lg">
                    {/* Quote icon */}
                    <div className="mb-6">
                    ...
                    <div className="flex items-center justify-center mb-6">
                      <div className="flex gap-1">
                        {[...Array(testimonial.stars)].map((_, i) => <Star key={i} className="w-5 h-5 fill-yellow-400 text-yellow-400" />)}
                      </div>
                    </div>
