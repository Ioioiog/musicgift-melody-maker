...
const Navigation = () => {
  ...
  return <>
      ...
      <header className="fixed top-2 sm:top-4 w-full bg-white z-50 border-b border-gray-100">
        <div className="container mx-auto px-2 sm:px-4 md:px-6 bg-transparent">
          <div className="flex items-center justify-between h-20 sm:h-24 md:h-28">
            
            {/* Logo - now part of the navigation flow */}
            <div className="flex-shrink-0 -mt-4 sm:-mt-6 md:-mt-8">
              <Link to="/" className="block">
                <img src="/lovable-uploads/9d0d10ef-2340-4632-8df0-f5058547a0c9.png" alt="MusicGift Logo" className="h-28 w-16 sm:h-32 sm:w-20 md:h-36 md:w-24 lg:h-40 lg:w-28 transition-transform duration-100 ease-in-out hover:scale-105" />
              </Link>
            </div>
            ...
            <div className="hidden lg:flex items-center space-x-3 xl:space-x-4 flex-shrink-0">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="bg-white border shadow-lg z-50">
                  {languages.map(lang => <DropdownMenuItem key={lang} onClick={() => setCurrentLanguage(lang)} className={`hover:bg-gray-100 ${currentLanguage === lang ? "bg-violet-50" : ""}`}>
                      {languageNames[lang]}
                    </DropdownMenuItem>)}
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            {/* Mobile Menu Toggle */}
            <button className="lg:hidden flex-shrink-0 p-2" onClick={() => setIsMenuOpen(!isMenuOpen)}>
