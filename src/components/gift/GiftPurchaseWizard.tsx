  ...
  if (!user) {
    return <Card className="bg-transparent border-transparent shadow-none">
        <CardContent className="p-6 text-center">
          ...
          <Button onClick={() => window.location.href = '/auth'} className="bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700">
            {t('signIn')}
          </Button>
        </CardContent>
      </Card>;
  ...
  return <Card className="bg-transparent border-transparent shadow-none">
      <CardHeader className="px-3 py-2">
        ...
        <div className="flex items-center space-x-2 mb-4">
          {steps.map((step, index) => <div key={step.key} className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${index <= currentStep ? 'bg-purple-600 text-white' : 'bg-purple-100 text-purple-400'}`}>
                {index + 1}
              </div>
              {index < steps.length - 1 && <div className={`w-8 h-0.5 mx-1 ${index < currentStep ? 'bg-purple-600' : 'bg-purple-200'}`} />}
            </div>)}
        </div>
      </CardHeader>
      ...
      <CardContent className="p-3">
          ...
          duration: 0.2
            ...
            {currentStep === 0 && <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {PRESET_AMOUNTS[currency as keyof typeof PRESET_AMOUNTS]?.map(amount => <Button key={amount} variant={selectedAmount === amount && !isCustomAmount ? "default" : "outline"} onClick={() => handleAmountSelect(amount)} className={`h-16 text-lg font-semibold ${selectedAmount === amount && !isCustomAmount ? 'bg-purple-600 text-white border-purple-600' : 'border-purple-200 text-purple-600 hover:bg-purple-50'}`}>
                      {amount} {currency}
                    </Button>)}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" value={customAmount} onChange={e => handleCustomAmountChange(e.target.value)} placeholder={`${t('enterAmountIn')} ${currency}`} className={`${isCustomAmount ? 'border-purple-600 bg-purple-50' : 'border-purple-200'}`} />
            ...
            {currentStep === 1 && <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={formData.sender_name} onChange={e => handleInputChange('sender_name', e.target.value)} placeholder={t('enterYourFullName')} className="border-purple-200" />
                  </div>
                  ...
                  <div>
                    ...
                    <Input type="email" value={formData.sender_email} onChange={e => handleInputChange('sender_email', e.target.value)} placeholder="your@email.com" className="border-purple-200" />
                  </div>
                  ...
                  <div>
                    ...
                    <Input value={formData.recipient_name} onChange={e => handleInputChange('recipient_name', e.target.value)} placeholder={t('enterRecipientName')} className="border-purple-200" />
                  </div>
                  ...
                  <div>
                    ...
                    <Input type="email" value={formData.recipient_email} onChange={e => handleInputChange('recipient_email', e.target.value)} placeholder="recipient@email.com" className="border-purple-200" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Textarea value={formData.message_text} onChange={e => handleInputChange('message_text', e.target.value)} placeholder={t('writeHeartfeltMessage')} className="border-purple-200 min-h-[80px]" rows={3} />
                </div>
                ...
                <div>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-full justify-start text-left font-normal border-purple-200">
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {deliveryDate ? format(deliveryDate, "PPP") : t('chooseDeliveryDate')}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0">
                      <Calendar mode="single" selected={deliveryDate} onSelect={setDeliveryDate} initialFocus disabled={{
                    before: new Date()
                  }} />
                    </PopoverContent>
                  </Popover>
            ...
            {currentStep === 2 && <div className="space-y-4">
                ...
                {designs.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {designs.filter(design => design.is_active).map(design => <div key={design.id} className={`cursor-pointer rounded-lg border-2 p-3 transition-colors ${selectedDesignId === design.id ? 'border-purple-600 bg-purple-50' : 'border-purple-200 hover:border-purple-300'}`} onClick={() => setSelectedDesignId(design.id)}>
                        <div className="aspect-video bg-gradient-to-br from-purple-100 to-pink-100 rounded mb-2 flex items-center justify-center">
                          {design.preview_image_url ? <img src={design.preview_image_url} alt={design.name} className="w-full h-full object-cover rounded" /> : <Gift className="w-8 h-8 text-purple-400" />}
                        </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex justify-center">
                    <GiftCardPreview design={selectedDesign} formData={formData} amount={getFinalAmount()} currency={currency} />
                  </div>
                </div>
              </div>}
        ...
        <div className="flex justify-between mt-6 pt-4 border-t border-purple-200">
          <Button onClick={handlePrev} disabled={currentStep === 0} variant="outline" className="border-purple-200 text-slate-50">
            <ArrowLeft className="w-4 h-4 mr-2" />
            {t('back')}
          </Button>

          {currentStep < steps.length - 1 ? <Button onClick={handleNext} disabled={!canProceed()} className="bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-orange-50">
              {t('next')}
              <ArrowRight className="w-4 h-4 ml-2" />
            </Button> : <Button onClick={handleSubmit} disabled={!canProceed() || isProcessing} className="bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700">
