...
const VideoHero = () => {
  ...
  return <section className={`video-hero relative overflow-hidden video-hero-optimized critical-resource ${isMobile ? '' : 'h-[60vh] sm:h-[70vh] md:h-[80vh] lg:h-screen'}`} style={isMobile && mobileHeight ? {
    height: mobileHeight
  } : {}}>
      ...
      <video key={language} ref={videoRef} autoPlay playsInline webkit-playsinline="true" preload="metadata" muted={!hasAudio} onEnded={handleVideoEnd} className={`absolute ${isMobile ? 'top-16' : 'top-0'} left-0 w-full ${isMobile ? 'h-auto' : 'h-full'} object-cover hw-accelerated`} poster={posterSrc}>
        {useWebM && <source src={videoWebM} type="video/webm" />}
        <source src={videoSrc} type="video/mp4" />
      </video>

      {/* Mobile Play Overlay */}
      {showPlayOverlay && isMobileDevice() && <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-20">
          <Button onClick={handlePlayWithSound} size="lg" className="bg-white/90 text-black rounded-full shadow-lg hover:bg-white transform hover:scale-105 transition-all duration-300 animate-pulse">
