...
const FormFieldRenderer: React.FC<FormFieldRendererProps> = ({
    ...
    switch (field.field_type) {
      ...
      case 'url':
        ...
        return <div className="space-y-1">
            <Input type={field.field_type} value={value || ''} onChange={e => isVatField ? handleVatCodeChange(e.target.value) : onChange(e.target.value)} placeholder={field.placeholder_key ? t(field.placeholder_key) : ''} required={field.required} className={cn("h-8 text-sm border-2 border-gray-300 bg-white focus:border-orange-500 focus:ring-orange-500 focus:bg-orange-50 transition-all duration-200", isVatField && vatError && "border-red-500 focus:border-red-500")} />
            {isVatField && vatError && <div className="flex items-center space-x-2 text-red-600 text-xs bg-red-50 p-1.5 rounded-md">
                <AlertCircle className="w-3 h-3" />
                <span>{t(vatError === 'Cod TVA prea scurt' ? 'vatCodeTooShort' : 'vatCodeInvalidFormat')}</span>
              </div>}
          </div>;
      case 'textarea':
        return <Textarea value={value || ''} onChange={e => onChange(e.target.value)} placeholder={field.placeholder_key ? t(field.placeholder_key) : ''} required={field.required} className="min-h-[60px] text-sm border-2 border-gray-300 bg-white focus:border-orange-500 focus:ring-orange-500 focus:bg-orange-50 transition-all duration-200" />;
      case 'audio':
        return <div className="space-y-1">
            ...
            {field.placeholder_key && <p className="text-xs text-gray-600 bg-gray-50 p-1.5 rounded-md">
      ...
      case 'select':
        if (!field.options || !Array.isArray(field.options)) {
          ...
          return <div className="flex items-center space-x-2 text-amber-600 bg-amber-50 p-2 rounded-md border border-amber-200">
              <AlertCircle className="w-3 h-3" />
        ...
        return <Select value={value || ''} onValueChange={onChange}>
            ...
            <SelectContent className="bg-white border-2 border-gray-300">
              ...
              return <SelectItem key={`${optionValue}-${index}`} value={optionValue} className="text-sm">
      ...
      case 'checkbox':
        return <div className="flex items-start space-x-2 bg-transparent rounded-lg p-1 hover:bg-white/5 transition-all duration-200 py-0">
            <Toggle pressed={value || false} onPressedChange={onChange} size="sm" className={`h-4 w-4 rounded-full p-0 transition-all duration-200 ${value ? 'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white shadow-lg' : 'bg-white/20 hover:bg-white/30 border border-gray-400 text-gray-600'}`}>
      ...
      case 'checkbox-group':
        ...
        if (filteredAddons.length === 0) {
          return <div className="text-xs text-gray-500 italic bg-gray-50 p-3 rounded-lg text-center">
              {t('noAddonsAvailable')}
            </div>;
        }
        return <div className="space-y-2">
            {filteredAddons.map(addon => <Card key={addon.id} className="border-2 border-gray-300 hover:border-orange-300 transition-all duration-200 shadow-sm hover:shadow-md">
                <CardContent className="p-2">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-2 flex-1">
                      <Checkbox checked={selectedAddons.includes(addon.addon_key)} onCheckedChange={checked => onAddonChange(addon.addon_key, checked as boolean)} className="mt-0.5" />
                      <div className="flex-1">
                        ...
                        {addon.description_key && <p className="text-xs text-gray-700 mb-2 leading-relaxed">
                            {t(addon.description_key)}
                          </p>}
                        
                        {/* Render addon-specific fields when selected */}
                        {selectedAddons.includes(addon.addon_key) && addon.trigger_field_type && <div className="mt-2 p-2 bg-orange-50 rounded-lg border border-orange-200">
                            {addon.trigger_field_type === 'file' && <div>
                                ...
                                <div className="border-2 border-dashed border-orange-300 rounded-lg p-2 text-center bg-white hover:border-orange-400 transition-colors">
                                  ...
                                  <label htmlFor={`file-${addon.addon_key}`} className="cursor-pointer text-xs text-orange-600 hover:text-orange-700 font-semibold">
      ...
      case 'date':
        return <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-full h-8 justify-start text-left font-normal border-2 border-gray-300 bg-white hover:border-orange-300 text-xs", !date && "text-muted-foreground")}>
  ...
  return <div className="space-y-1">
      <Label className="text-xs font-bold text-gray-800 block my-[6px] py-0">
