  ...
  return <div className="space-y-1.5">
      ...
      <div className="mb-1">
        ...
        <div className="space-y-1">
          {contactLegalStepFields.filter(field => ['fullName', 'email', 'phone', 'address', 'city'].includes(field.field_name)).sort((a, b) => a.field_order - b.field_order).map(field => <Card key={field.id} className="">
      ...
      <div className="mb-1">
        ...
        <div className="space-y-1">
          {contactLegalStepFields.filter(field => ['invoiceType', 'companyName', 'vatCode', 'registrationNumber', 'companyAddress', 'representativeName'].includes(field.field_name)).sort((a, b) => a.field_order - b.field_order).map(field => <Card key={field.id} className="bg-gradient-to-br from-orange-500/20 to-purple-600/20 backdrop-blur-sm border border-orange-400/30 hover:border-orange-300/50 transition-colors">
                <CardContent className="p-1.5">
                  <FormFieldRenderer field={field} value={formData[field.field_name]} onChange={value => onInputChange(field.field_name, value)} selectedAddons={selectedAddons} onAddonChange={onAddonChange} availableAddons={availableAddons} addonFieldValues={addonFieldValues} onAddonFieldChange={onAddonFieldChange} selectedPackage={selectedPackage} selectedPackageData={selectedPackageData} formData={formData} />
                </CardContent>
              </Card>)}
      ...
      <div>
        ...
        <div className="space-y-1">
          {contactLegalStepFields.filter(field => ['acceptMentionObligation', 'acceptDistribution', 'finalNote'].includes(field.field_name)).sort((a, b) => a.field_order - b.field_order).map(field => <Card key={field.id} className="bg-gradient-to-br from-orange-500/20 to-purple-600/20 backdrop-blur-sm border border-orange-400/30 hover:border-orange-300/50 transition-colors">
                <CardContent className="p-1.5">
                  <FormFieldRenderer field={field} value={formData[field.field_name]} onChange={value => onInputChange(field.field_name, value)} selectedAddons={selectedAddons} onAddonChange={onAddonChange} availableAddons={availableAddons} addonFieldValues={addonFieldValues} onAddonFieldChange={onAddonFieldChange} selectedPackage={selectedPackage} selectedPackageData={selectedPackageData} formData={formData} />
                </CardContent>
              </Card>)}
