  ...
  if (isMobile) {
    return <div className={cn("mb-8", className)}>
        ...
        <div className="flex justify-center mt-4 space-x-2">
          {steps.map((step, index) => <div key={step.number} className={`w-2 h-2 rounded-full transition-all duration-300 ${step.isCompleted ? 'bg-green-500' : step.isCurrent ? 'bg-orange-500 scale-125' : 'bg-gray-300'}`} />)}
        </div>
      </div>;
  }

  // Desktop layout (existing design)
  return <div className={cn("mb-12", className)}>
