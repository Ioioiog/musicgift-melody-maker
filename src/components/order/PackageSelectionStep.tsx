          ...
          <CardContent className="p-2">
            <Select value={selectedPackage} onValueChange={onPackageSelect}>
                ...
                <SelectValue placeholder={<div className="flex items-center text-white/80 text-sm">
                  ...
                  {selectedPackageData && <div className="flex items-center justify-between w-full pr-3">
                      <div className="flex items-center space-x-2">
                        {selectedPackageData.tag && <Badge className={`${getTagColor(selectedPackageData.tag)} text-xs px-1.5 py-0.5 font-medium`}>
              ...
              <SelectContent className="w-full bg-white/95 backdrop-blur-md border-2 border-purple-200/50 shadow-2xl z-[9999] rounded-xl">
                {packages.map(pkg => <SelectItem key={pkg.value} value={pkg.value} className="p-3 hover:bg-gradient-to-r hover:from-purple-50 hover:to-pink-50 focus:bg-gradient-to-r focus:from-purple-50 focus:to-pink-50 cursor-pointer border-b border-gray-100/80 last:border-b-0 rounded-lg mx-1 my-0.5 transition-all duration-200">
                    <div className="flex items-center justify-between w-full">
                      <div className="flex items-center space-x-2 flex-1">
                        {pkg.tag && <Badge className={`${getTagColor(pkg.tag)} text-xs px-1.5 py-0.5 shrink-0 font-medium`}>
                            <div className="flex items-center gap-1">
                              {getTagIcon(pkg.tag)}
                              <span>{t(pkg.tag)}</span>
                            </div>
            ...
            <CardContent className="p-2">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  {selectedPackageData.tag && <Badge className={`${getTagColor(selectedPackageData.tag)} mt-0.5 text-xs px-1.5 py-0.5 font-medium`}>
                      <div className="flex items-center gap-1">
                        {getTagIcon(selectedPackageData.tag)}
                        <span>{t(selectedPackageData.tag)}</span>
                      </div>
