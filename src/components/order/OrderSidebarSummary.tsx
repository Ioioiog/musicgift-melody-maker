...
const OrderSidebarSummary: React.FC<OrderSidebarSummaryProps> = ({ orderData, giftCard }) => {
  ...
  if (!orderData?.selectedPackage) {
    return (
      <div className={isMobile ? "mb-4" : "lg:sticky lg:top-4"}>
            ...
            {selectedPackageData.includes && (
              <div className="mt-3 sm:mt-4 p-2 sm:p-3 bg-white/5 rounded-lg border border-white/10">
                ...
                <ul className="space-y-1">
                  {selectedPackageData.includes.slice(0, isMobile ? 2 : 3).map((includeItem, index) => (
                  ...
                  {selectedPackageData.includes.length > (isMobile ? 2 : 3) && (
