...
const ContentFreshnessManager = () => {
  ...
  useEffect(() => {
    const checkContentFreshness = () => {
      ...
      if (daysSinceUpdate < 7) freshness = 'fresh';
      else if (daysSinceUpdate < 30) freshness = 'moderate';
  ...
  if (process.env.NODE_ENV === 'development' && contentInfo) {
    return (
      <div className="fixed top-4 left-4 bg-white border border-gray-200 rounded-lg p-3 shadow-lg text-xs z-50 max-w-xs">
        <div className="font-bold mb-2">Content Freshness</div>
        <div className={`px-2 py-1 rounded text-center ${
          contentInfo.freshness === 'fresh' ? 'bg-green-100 text-green-800' :
          contentInfo.freshness === 'moderate' ? 'bg-yellow-100 text-yellow-800' :
