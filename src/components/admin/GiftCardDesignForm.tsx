...
const GiftCardDesignForm: React.FC<GiftCardDesignFormProps> = ({
  design,
  onSuccess
}) => {
  ...
  return <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[450px] bg-white border-r flex flex-col overflow-hidden">
          <div className="flex-1 overflow-auto">
            ...
            <div className="p-6 border-b">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="name" value={formData.name} onChange={e => handleInputChange('name', e.target.value)} placeholder="Enter design name" className="mt-1" required />
                </div>
                ...
                <div>
                  ...
                  <Input id="theme" value={formData.theme} onChange={e => handleInputChange('theme', e.target.value)} placeholder="e.g., Birthday, Christmas" className="mt-1" required />
                </div>
                ...
                <div>
                  ...
                  <ImageUpload label="" value={formData.preview_image_url} onChange={url => handleInputChange('preview_image_url', url)} bucketName="gift-card-designs" maxSizeBytes={5 * 1024 * 1024} acceptedTypes={['image/jpeg', 'image/png', 'image/webp', 'image/gif']} />
                </div>
            ...
            <Collapsible open={!collapsedSections.canvasInfo} onOpenChange={() => toggleSection('canvasInfo')}>
              <CollapsibleTrigger className="w-full">
                <div className="p-6 border-b flex items-center justify-between hover:bg-gray-50">
                  ...
                  {collapsedSections.canvasInfo ? <ChevronRight className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                </div>
              </CollapsibleTrigger>
              <CollapsibleContent>
            ...
            <Collapsible open={selectedElement ? !collapsedSections.properties : false} onOpenChange={() => selectedElement && toggleSection('properties')}>
              <CollapsibleTrigger className="w-full" disabled={!selectedElement}>
                <div className={`p-6 border-b flex items-center justify-between ${selectedElement ? 'hover:bg-gray-50' : 'opacity-50'}`}>
                  ...
                  {selectedElement && (collapsedSections.properties ? <ChevronRight className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />)}
                </div>
              </CollapsibleTrigger>
              <CollapsibleContent>
                {selectedElement && <div className="p-6">
                    <div className="space-y-4">
                      <div className="text-sm font-medium text-gray-700 mb-3 p-2 bg-blue-50 rounded">
                        ...
                        {selectedElement.type === 'placeholder' && <span className="text-blue-600 ml-2">(Placeholder)</span>}
                      </div>

                      {/* Text/Placeholder Element Properties */}
                      {(selectedElement.type === 'text' || selectedElement.type === 'placeholder') && <div className="space-y-4">
                          <div>
                            <Label className="text-sm font-medium text-gray-700">
                              {selectedElement.type === 'placeholder' ? 'Placeholder Text' : 'Text Content'}
                            </Label>
                            <Input value={selectedElement.text || ''} onChange={e => updateSelectedElementProperty('text', e.target.value)} className="mt-1" placeholder={selectedElement.type === 'placeholder' ? 'e.g., Recipient Name' : 'Enter text'} />
                          </div>
                          
                          <ColorPicker label="Text Color" value={selectedElement.color || selectedElement.fill || '#000000'} onChange={color => {
                      ...
                      updateSelectedElementProperty('fill', color);
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <Input type="number" value={selectedElement.fontSize || 16} onChange={e => updateSelectedElementProperty('fontSize', parseInt(e.target.value))} className="mt-1" min="8" max="72" />
                            </div>
                            
                            <div>
                              <Label className="text-sm font-medium text-gray-700">Font Weight</Label>
                              <select value={selectedElement.fontWeight || 'normal'} onChange={e => updateSelectedElementProperty('fontWeight', e.target.value)} className="w-full h-10 px-3 border rounded mt-1">
                      ...
                      {(selectedElement.type === 'rectangle' || selectedElement.type === 'rounded-rectangle' || selectedElement.type === 'circle') && <div className="space-y-4">
                          <ColorPicker label="Fill Color" value={selectedElement.color || selectedElement.fill || selectedElement.backgroundColor || '#cccccc'} onChange={color => {
                      updateSelectedElementProperty('color', color);
                      updateSelectedElementProperty('fill', color);
                      updateSelectedElementProperty('backgroundColor', color);
                    }} />
                          
                          <ColorPicker label="Border Color" value={selectedElement.strokeColor || selectedElement.stroke || selectedElement.borderColor || '#000000'} onChange={color => {
                      ...
                      updateSelectedElementProperty('borderColor', color);
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <Input type="number" value={selectedElement.strokeWidth || 1} onChange={e => updateSelectedElementProperty('strokeWidth', parseInt(e.target.value))} className="mt-1" min="0" max="20" />
                            </div>
                      ...
                      {selectedElement.type === 'line' && <div className="space-y-4">
                          <ColorPicker label="Line Color" value={selectedElement.color || selectedElement.stroke || '#000000'} onChange={color => {
                      updateSelectedElementProperty('color', color);
                      updateSelectedElementProperty('stroke', color);
                    }} />
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <Input type="number" value={selectedElement.strokeWidth || 2} onChange={e => updateSelectedElementProperty('strokeWidth', parseInt(e.target.value))} className="mt-1" min="1" max="20" />
                            </div>
                      ...
                      <div className="pt-4 border-t">
                        <Button onClick={() => setSelectedElementIndex(null)} variant="outline" size="sm" className="w-full">
                          Deselect Element
                        </Button>
                      </div>
                    </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-auto p-6">
            <div className="h-full flex items-center justify-center px-0">
              <div className="bg-gradient-to-br from-white to-gray-50/50 rounded-2xl shadow-xl border border-purple-200/30 backdrop-blur-sm overflow-hidden group hover:shadow-2xl transition-all duration-500 mx-0 my-0 px-0">
                ...
                <div className="relative z-10 p-8 px-[45px] py-[22px] mx-[4px] my-[3px]">
                  <GiftCardCanvasEditor value={formData.template_data} onChange={handleCanvasDataChange} backgroundImage={formData.preview_image_url} selectedElementIndex={selectedElementIndex} onElementSelect={setSelectedElementIndex} />
                </div>
              </div>
            </div>
          </div>
      ...
      {(createDesignMutation.isPending || updateDesignMutation.isPending) && <div className="fixed bottom-4 right-4 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2">
