...
const PaymentSuccess = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      if (isGiftCardRequest && giftCardId) {
        ...
        try {
          ...
          if (error) {
            ...
            toast({
              title: 'Error loading gift card details',
              description: 'Could not retrieve your gift card information.',
              variant: 'destructive'
            });
          } else {
      ...
      } else if (isQuoteRequest && quoteId) {
    ...
    // Poll for payment status updates in case webhook is delayed (only for orders and gift cards)
    if ((!isQuoteRequest && orderId) || (isGiftCardRequest && giftCardId)) {
        ...
        <div className="flex items-center justify-center min-h-[60vh] relative z-10">
          <div className="text-center">
            ...
            <p className="text-white/90 text-lg font-medium">
              {isGiftCardRequest ? 'Se încarcă detaliile gift card-ului...' : 
               isQuoteRequest ? 'Se încarcă detaliile cererii de ofertă...' : 
                    ...
                    <div className="relative z-10">
                      <div className="mb-8">
                        ...
                        <h1 className="text-3xl md:text-4xl font-bold text-white mb-4 bg-gradient-to-r from-green-300 to-purple-300 bg-clip-text text-transparent">
                          {paymentVerified ? 'Gift Card-ul a fost creat cu succes!' : 'Gift Card-ul este în procesare'}
                        </h1>
                        <p className="text-white/80 text-lg md:text-xl leading-relaxed max-w-2xl mx-auto">
                          {paymentVerified ? 
                            'Vă mulțumim pentru achiziție. Gift card-ul a fost creat și va fi livrat destinatarului.' : 
                            'Gift card-ul dumneavoastră este în curs de procesare. Vă vom informa când va fi gata.'}
                        </p>
                      </div>
                        ...
                        <motion.div initial={{
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div className="flex justify-between items-center py-2 border-b border-white/10">
                              <span className="text-white/70">ID Gift Card:</span>
                              <span className="font-mono text-white bg-white/10 px-2 py-1 rounded">#{giftCardDetails.id.slice(0, 8)}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-white/10">
                              <span className="text-white/70">Valoare:</span>
                              <span className="font-semibold text-green-300">
                                {formatCurrency(giftCardDetails.gift_amount, giftCardDetails.currency || 'RON')}
                              </span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-white/10">
                              <span className="text-white/70">Destinatar:</span>
                              <span className="text-white">{giftCardDetails.recipient_name}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-white/10">
                              <span className="text-white/70">Email destinatar:</span>
                              <span className="text-white">{giftCardDetails.recipient_email}</span>
                            </div>
                      ...
                      <div className="space-y-6">
                        <p className="text-white/70 leading-relaxed">
                          {paymentVerified ? 
                            'Gift card-ul a fost trimis destinatarului prin email și este gata de utilizare.' : 
                            'Vă vom informa prin email când gift card-ul va fi finalizat și trimis destinatarului.'}
                        </p>
