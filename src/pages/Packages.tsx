...
const Packages = () => {
  ...
  const renderPackagePrice = (pkg: any) => {
    ...
    return <div className="text-3xl font-bold text-white">
      ...
      backgroundRepeat: 'no-repeat'
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {packages?.map((pkg, index) => <motion.div key={pkg.value} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            delay: index * 0.1
                ...
                <Card className={`h-full transition-all duration-300 bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 hover:shadow-xl ${hoveredCard === pkg.value ? 'scale-105' : ''} ${pkg.tag === 'popular' ? 'ring-2 ring-purple-500' : ''}`}>
                  <CardHeader className="relative">
                    {pkg.tag === 'popular' && <Badge className="absolute -top-2 -right-2 bg-purple-500 hover:bg-purple-600">
